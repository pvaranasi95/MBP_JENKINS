pipeline {
    ...
    stages {
        ...
        stage ('Git checkout') {
            steps{
                checkout scmGit(branches: [[name:'%JOB_NAME%']], extensions: [], userRemoteConfigs: [[credentialsId: 'pvaranasi-dockerhub', url: 'https://github.com/pvaranasi95/MBP_JENKINS.git']])
            }
        }
        stage('Sonar scan') {
            steps{
